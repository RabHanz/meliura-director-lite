...
One brief, the kind a person can work from. A report, a gallery and a list of links with
adjectives on them are all easier to produce and much less use.

Write it for someone who will read the top of it once and go and make something. The six sections
come in this order, with the most usable first. The research notes go at the bottom, under the
briefs.
...
One post, briefed completely, and one sentence on why this one comes before the others. Pick it.
Don't hand the choice back.

> **Make this first: "Open it on day ___."** (The blank is the roaster's own resting window.)
> **Why this one first:** resting is the live argument in the home-brewing threads this fortnight
> (§4, line 2), and the construction for it (§2, first entry) needs nothing the shop doesn't
> already have: one number, a flat ground, no photograph.
> **Format:** four-slide carousel.
> **Cover:** one number, the days the roaster rests a bag before brewing it, on the shop's cream
> ground, nothing else.
...
> number staying unexplained for one beat.

The pick should be the post with the best evidence behind it that this creator can make this week
with what they have. If those two things pulled apart, say which one decided it.

---

## 2. It works in another niche. Here is your version.
...
> both. The number carries no data here. It is the image.
>
> **Your version:** one number per card, the resting days for this week's bag, set at four or five
> times the size of the line beneath it, on a flat cream field, nothing else in the frame. The
> number is the image.
>
> **Slides:** cover, the number alone. 2, what happens inside the bag during those days. 3, the
...
**Include at least one refusal, with its reason.** A run that found nothing to refuse either got
lucky or wasn't testing. Same shape:

> **Refused:** a skate shop's slow-motion trick reel. `medium does not carry`: the value is the
> timing of the landing, and this creator makes carousels and still posts. A carousel cannot
> inherit a slow-motion landing.
...
Six to ten lines. Each names a subject or a question being published and argued about in this
niche **right now**, with the page you read it on and that page's date.

> **Resting coffee, argued as a freshness problem.** Three threads in the last fortnight from
> people who brewed a bag the day it arrived and blamed the roaster (a home-brewing forum, threads
> dated `<date>` to `<date>`). The frustration is specific: they bought fresh on purpose and the
> coffee tasted worse for it.
...
## 5. Well made, and what we could establish about it

Up to eight references, each labelled **seen**, **described** or **counted** (`THE-METHOD.md` §3).
Each one has:

- **the URL you opened**, and the date you opened it
...
> about a fifth of it, and a photographed paper texture underneath at low contrast so the type sits
> on it without a box or an outline. Nothing else is in the frame. The size jump between the name
> and the notes does all the hierarchy work. There is no colour doing it, no rule line, no icon.
...
A short table. Every row says **where the claim comes from**, and rows with different bases are
kept apart rather than sorted together as if they were the same kind of thing.

| term | why it is here | basis | when |
|---|---|---|---|
| natural process | what the niche calls it; on most roaster accounts I opened | seen on 8 of 10 accounts | `<date>` |
| resting | rising in community threads | 3 threads in a fortnight, one forum | `<date>` to `<date>` |
| latte art | adjacent, high traffic, different audience | relative interest, Google Trends, 12 months | `<date>` |

**There is no volume column** unless you actually read a volume off a page. Relative interest is
relative interest and is labelled that way.

---

## Research notes

These go below the briefs, because the creator reads them second. They tell the reader how much
weight the rest can take.
...
Every claim that is a reading rather than something you saw.
...
Each one is the difference between a brief and a plausible document, and a plausible document is
worse than nothing because someone will act on it.

1. **No invented number, ever.** No view count, save count, follower count, growth percentage,
   engagement rate or ratio that you did not read off a page you opened, and none put in the
   creator's mouth in a hook. If the method wants a number and you don't have one, write
   **unknown**. Unknown appearing several times means the brief is honest. It does not mean the
   brief is thin.
...
3. **Label inference as inference**, inline and again in the notes. "This looks aimed at home
   ...
   useful. Mixing them up is not.

4. **Never describe a picture you did not see.** On a text-only host, craft comes from the
...
7. **Refuse rather than fill.** An empty section with a stated reason is a finding. A section
   filled with something adjacent is a lie with good manners.

8. **Never write the post.** Brief it. Once you draft the caption, the work has moved from the
   creator to the machine, and the audience can tell.
